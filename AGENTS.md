...
- The `ai/` module is still an intentional placeholder (`pass` stubs); `kubernetes/` and `services/investigation.py` are now implemented (evidence gathering only, no AI). Do not break the health endpoint or the run/build flow when adding AI reasoning later.
